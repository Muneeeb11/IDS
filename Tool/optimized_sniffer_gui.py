...
class OptimizedPacketSnifferGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.packet_buffer = deque(maxlen=500)  # Keep aligned with visible table rows
        self.displayed_packets = deque(maxlen=500)
        self.threat_items = {}
    ...
    def get_prediction_batch_size(self):
        """Map the UI speed mode to batch size for model throughput."""
        speed_mode = self.speed_combo.currentText() if hasattr(self, 'speed_combo') else "Balanced"
        return {
            "Fast": 32,
            "Balanced": 16,
            "Detailed": 8,
    ...
    def on_speed_mode_changed(self, *_):
        """Apply speed-mode changes to the running predictor immediately."""
        if self.async_predictor:
            self.async_predictor.batch_size = self.get_prediction_batch_size()
            self.statusBar().showMessage(
                f"Prediction mode set to {self.speed_combo.currentText()} "
                f"(batch size {self.async_predictor.batch_size})"
            )

    def get_live_efficiency(self):
        """Return live processing efficiency as a percentage."""
        if self.packets_received <= 0:
            return 0.0
        return min(100.0, (self.packets_processed / self.packets_received) * 100.0)
    ...
    def load_ids_model(self):
        ...
        try:
            ...
            # The app can be launched from the repo root or from Tool/, so make
            # the runtime imports independent of the current working directory.
            ...
            # Load the training class straight from disk so the packaged runtime
            # does not depend on installing `scripts` as a Python package.
            ...
            try:
                # Keep supporting a few legacy layouts so older demo folders can
                # still start even if they were copied around manually.
                possible_paths = [
    ...
    def setup_control_panel(self, layout):
        ...
        self.speed_combo.currentIndexChanged.connect(self.on_speed_mode_changed)
    ...
    def setup_packet_table(self, splitter):
        ...
        self.stats_labels = {
            ...
            'efficiency': QLabel("📈 Efficiency: 0.0%")
        }
    ...
    def setup_sniffer(self):
        ...
        # Worker threads never touch Qt widgets directly. They emit signals and
        # the main GUI thread performs all widget updates safely.
    ...
    def on_attack_detected(self, packet):
        ...
        # Repeated modal dialogs block the event loop and make the UI feel
        # frozen, so serious alerts are throttled.
        if (
            packet['threat_level'] in ['DDoS', 'DoS']
            and time.time() - self.last_alert_time >= self.alert_cooldown_seconds
        ):
    ...
    def add_packet_to_table(self, packet):
        ...
        if self.packet_table.rowCount() >= 500:
            ...
            if self.displayed_packets:
                old_packet = self.displayed_packets.popleft()
                self.threat_items.pop(id(old_packet), None)
        ...
        self.threat_items[id(packet)] = threat_item
    ...
    def show_packet_details(self, index):
        ...
        row = index.row()
        if row < 0 or row >= len(self.displayed_packets):
            return
            
        packet = list(self.displayed_packets)[row] if row < len(self.displayed_packets) else None
        if not packet:
            return
    ...
    def update_display(self):
        ...
        if self.async_predictor:
            ...
            for packet in results:
                ...
                threat_item = self.threat_items.get(id(packet))
                if threat_item is not None:
                    threat_item.setText(threat)
                    self.color_threat_item(threat_item, threat)
    ...
    def update_stats(self):
        ...
        self.stats_labels['efficiency'].setText(
            f"📈 Efficiency: {self.get_live_efficiency():.1f}%"
        )
        ...
        try:
            ...
            if os.path.exists(log_dir):
                ...
                if log_files:
                    ...
                    if latest_log != self.last_log_file:
                        self.last_log_file = latest_log
                        self.last_log_position = 0
                        self.log_text.clear()
                    ...
                    if new_text:
                        ...
                        self.log_text.insertPlainText(new_text)
                        self.log_text.moveCursor(self.log_text.textCursor().End)
        except Exception:
...
• Live Efficiency: {self.get_live_efficiency():.1f}% (Processed / Received)
