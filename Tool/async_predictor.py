...
class AsyncPredictor:
    ...
    def __init__(self, model, batch_size=10, max_queue_size=100):
        ...
        # `max_queue_size=0` means unbounded queueing. This keeps short capture
        # bursts such as localhost Nmap scans from being silently dropped.
    ...
    def _process_batch(self, batch, processing_times):
        """Classify one batch and push finished packets back to the GUI queue."""
        ...
        try:
            ...
            if features_batch:
                ...
                for i, (packet, prediction) in enumerate(zip(valid_packets, predictions)):
                    ...
                    # Be conservative in the live demo: weak non-benign scores
                    # create noisy false positives that are distracting in class.
                    if prediction != "Benign" and confidence < 0.7:
                        prediction = "Benign"

                    # The scan heuristic is allowed to override the ML label so
                    # obvious multi-port probing still surfaces as Recon live.
    ...
    def _detect_scan_behavior(self, packet):
        ...
        # Keep only the recent sliding window so a few old probes do not keep a
        # host flagged forever once the scan has stopped.
